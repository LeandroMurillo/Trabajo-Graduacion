...
Para el desarrollo del sistema se optó por una arquitectura basada en JavaScript/TypeScript en todas las capas de la aplicación (full-stack JS), lo que permite unificar el lenguaje y compartir lógicas de tipado y validación. Para la persistencia de datos se seleccionó MariaDB, un motor de base de datos relacional robusto. En el _backend_ se utiliza el entorno Node.js con el _framework_ Express.js, mientras que para las interfaces de usuario (_frontend_) se emplea la librería React.js.
...
- Toolpad: Es una librería de nueva generación que facilita la creación rápida de interfaces de gestión interna (dashboards, CRUDs) integrándose nativamente con Material UI, acelerando el desarrollo de herramientas administrativas complejas.
